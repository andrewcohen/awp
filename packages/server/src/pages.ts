 ...
 * make.
 *
 * Two, and no more. `gadget://` was briefly a third — accepted here and drawn
 * by the renderer rather than navigated to — and it is now a panel of its own
 * with a list behind it, because a thread accumulates gadgets and this feed
 * holds one address: every gadget written destroyed the one before it. The
 * guard is back to the shape it can be argued about in, which is the reason to
 * mention a scheme it no longer takes.
 */
