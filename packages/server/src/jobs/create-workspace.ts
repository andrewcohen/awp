      ...
      Effect.gen(function* () {
        ...
        yield* mux
          .start({
            ...
            command: input.agent,
            // Names the workspace to what runs in it. The status hooks that
            // once read these are gone — the chat is the status source now,
            // see `workspace-state.ts` — but the variables cost nothing.
            //
