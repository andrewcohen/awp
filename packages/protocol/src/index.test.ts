...
const handlers = AwpRpcs.toLayer({
  ...
  ServiceList: () => Effect.succeed([]),
  ServiceStart: (one) => Effect.succeed(`awp.${one.project}.${one.workspace}.service_${one.name}`),
  ServiceStop: () => Effect.void,
  ...
  MessageSend: ({ to, body }) =>
    Effect.succeed({
      id: "m20260916-0001",
      thread: thread.id,
      from: { project: "thicket", workspace: "lantern" },
      to: { project: "thicket", workspace: to },
      body,
      sentAt: 1_787_000_000_000,
      notifiedAt: undefined,
      readAt: undefined,
    }),
  MessageInbox: () => Effect.succeed([]),
  MessageList: () => Effect.succeed([]),
  MessageChanges: () => Stream.make([]),
