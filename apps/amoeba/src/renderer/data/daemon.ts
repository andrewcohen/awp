...
/**
 * Give a thread a different title.
 *
 * The title is written once, by a model, out of the sentence somebody typed
 * into the new-thread modal — so it is frequently almost right, and until this
 * there was no way to fix it. The call has been on the wire since threads
 * landed; what was missing was a caller.
 */
export const renameThread = (thread: string, title: string): Promise<Thread> =>
  runtime.runPromise(Effect.flatMap(AwpClient, (rpc) => rpc.ThreadRename({ thread, title })));
