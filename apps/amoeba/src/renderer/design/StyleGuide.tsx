...
import { Amoeba } from "../shell/Amoeba";
import { AppearanceToggle } from "./Appearance";
import type { ChatConfigOption } from "@awp-kit/protocol";
import { Transcript, Working } from "../panels/Chat";
import { Chip } from "./Chip";
import { type Command, agentCommands } from "@awp-kit/protocol/commands";
import { Composer, SessionBar } from "../panels/Composer";
import type { Item } from "../panels/conversation";
...
const CALLS: ReadonlyArray<Item> = [
  ...
  },
  {
    ...
    //
    // An edit answers with no output at all, so before the patch was on the
    // wire this row was a title and a tick — the row for the one kind of call
    // that actually changed something said the least about it. The patch is
    // the daemon's; see `ChatDiff`. Here it is written out, because a fixture
    // exists to be looked at without an agent behind it.
    diffs: [
      {
        path: "/repo/apps/amoeba/src/renderer/Fence.tsx",
        patch: [
          "Index: Fence.tsx",
          "===================================================================",
          "--- Fence.tsx",
          ...
          '-  const items = parsePatchFiles(source, "message");',
          '+  const items = safely(() => parsePatchFiles(source, "message"));',
          " ",
          "   if (items.length === 0) {",
