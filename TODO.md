...
37 open, as of 2026-09-17. The count is taken from the entries rather than
carried forward — it read "46 open" over 51 of them once.

Closed on 2026-09-17: #135 (gadgets — `awp_gadget` takes MDX, `gadgets.ts`
compiles it, and `Gadget.tsx` runs it in the accessory column). Its three open
questions were decided by building it. **MDX compiles in the daemon**, because
a document that does not compile is then a refusal handed back to the agent
still holding the source rather than a blank panel an hour later. **A thread has
many named gadgets and its page names one** — the address is
`gadget://<thread>/<name>` and rides the existing page feed, so no list, no
strip and no title field, none of which anything renders yet. **Liveness was not
attempted**: a gadget is read when the panel mounts and re-read when the agent
writes it again, which is what the showing's `at` is carried down for. What is
still nothing is durability — the daemon holds them in memory, and a restart
answers with a sentence rather than an empty column — and the app-native half of
the scope, which is `React` and the three token groups and deliberately nothing
else.
