...
Under `awp zdeck` only the **agent** pane clears it. The badge means the agent produced output you have not seen, and its pane is the only surface that shows it; a tmux window switch cleared the badge because it put you in the session with the agent one key away, which is not true of a pane.
