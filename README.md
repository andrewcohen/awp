...
**A local reply is not published.** Replies are the conversation you and the agent have about a finding; a batched review creates new threads only, so sending one would post it as a fresh top-level comment divorced from what it answers. They're counted as skipped. Replying into a GitHub thread needs that thread's own id and its own mutation. |
