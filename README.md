...
| `c` / `C` | Review the change. `c` opens awp's **own diff viewer in the deck** — the same viewer as [`awp diff`](#cli-reference-highlights), scoped to the selected workspace's working copy, rendered in place of the row list. The right pane is **one continuous scroll over the whole change** — every file's hunks in sequence, with an inline header opening each file — so reading a change never means stepping in and out of a per-file view. The left pane is a **jump index**: moving it seeks the stream to that file, and scrolling the stream highlights whichever file you've scrolled into. The diff pane is focused when the view opens. A **line cursor** marks where you are, highlighted vim-style across the full width — `j`/`k` move it a row, `ctrl+u`/`ctrl+d` a half page, `{`/`}` jump it to the previous/next hunk *anywhere in the change* (not just the current file), `g`/`G` to the start and end. The viewport follows the cursor rather than the reverse, keeping a couple of rows of lookahead, and the file divider highlights whichever file the cursor is in. `tab`/`shift+tab` switches pane (`enter` on a file also drills into the diff), `h`/`l` pans horizontally so a long truncated line can be read without wrapping, with `0`/`$` jumping to the line start and end (line numbers and the `+`/`-` gutter stay pinned; all no-ops under wrap), `c` leaves a **comment** on the cursor's line (`enter` saves, `ctrl+s` also sends it to the agent, `esc` discards), `e` opens the file in `$EDITOR` at the cursor's line, `w` toggles line wrap (off by default — long lines are truncated at the pane edge), `r` marks the file at the cursor **reviewed** and collapses it to its divider — so working through a change is read, `r`, next, and what's left in the stream is what you haven't cleared. The mark is keyed to the file's content, so if the agent edits a file after you reviewed it, it comes back rather than staying hidden. `ctrl+r` forces a refresh (the view also refreshes itself every couple of seconds, keeping your cursor on the line it was on), `/` filters files, `esc`/`q`/`c` closes. Note `w` means wrap *inside* the viewer; the deck's watch overlay `w` applies in row mode. Virtual inbox rows and workspaces still being created have no working copy to diff, so `c` falls back to the tuicr window for them. `C` still opens a **tuicr window** reviewing the whole change **against its stack base** — awp resolves the base to the nearest stacked-parent bookmark (the closest bookmarked ancestor of `@` that is neither trunk nor the workspace's own bookmark), falling back to `trunk()` (jj's auto-resolved default branch) when nothing is stacked, then runs `tuicr -r '<base>..@'`. So a stacked change is reviewed against its parent instead of the whole stack, and a non-stacked change is reviewed against the real trunk regardless of whether it's named `main`, `master`, or something else. |
| `v` | VCS window (`jjui`) |
| `s` | Shell window |
