...
- `~/.claude/settings.json` — hooks that report state to awp on `SessionStart` (idle), `UserPromptSubmit` / `PreToolUse` / `PostToolUse` (working), `Stop` (idle), and `PermissionRequest` / `Elicitation` (waiting). The `UserPromptSubmit` hook also pipes the prompt JSON into `awp internal report-status --prompt-stdin` so the deck can show the active prompt under each workspace. The `PreToolUse` hook passes `--waiting-when-tool AskUserQuestion` so the row flips to `waiting` while the agent is paused on an `AskUserQuestion` call; `PostToolUse` flips it back to `working` once the answer is in. `PermissionRequest` (a permission dialog is up — approve/deny a Bash command or file write) and `Elicitation` (an MCP server is requesting form input) are the dedicated "blocked on you" events: they badge the row `waiting` even when desktop notifications aren't configured. awp deliberately does **not** hook `Notification`: Claude fires it for its ~60s idle ping as well as permission prompts, so mapping it to `waiting` lit up the unread summary with false `▲` triangles for agents that had simply finished their turn — the dedicated `PermissionRequest` event covers the real case. Any stale awp-managed `Notification` hook from an older awp version is removed automatically on the next deck open. Unknown events are ignored by older Claude Code builds, so these are safe to install regardless of client version.
  ...
  For repos with a [`dev_loop`](#dev_loop), two more matcher-scoped entries enforce the loop: `PostToolUse(Bash)` → `awp gate record` and `PreToolUse(TaskUpdate)` → `awp gate check --hook`. They coexist with the matcher-less status entries above and no-op for repos without a `dev_loop`. See [`dev_loop` → Enforcement](#dev_loop).
...
| `awp gate check [--workspace <ws>]` | Self-check the current unit against its [`dev_loop`](#dev_loop) gates: exit 0 when every configured gate is green (ready to mark the unit complete), else a non-zero exit naming the blocking gate and the command to re-run. The hook variants (`awp gate record`, `awp gate check --hook`) are installed by `awp init hooks` to enforce the loop — see [`dev_loop` → Enforcement](#dev_loop). |
...
> Note: the pass/fail signal in the `PostToolUse` payload isn't guaranteed across Claude Code versions. `awp gate record` reads it defensively and, when the payload carries no clear signal, assumes the completed command passed (biasing away from false completion blocks); the transcript reconciler corrects a genuine failure on the next deck open.
...
For repos with a [`dev_loop`](#dev_loop), `awp init hooks` also installs two matcher-scoped Claude hooks that enforce the loop (they no-op elsewhere): `PostToolUse(Bash)` → `awp gate record` (records each gate's pass/fail into the workspace snapshot, event-driven) and `PreToolUse(TaskUpdate)` → `awp gate check --hook` (resets a unit's gates when it goes `in_progress`, and blocks marking it `completed` until its gates are green). Unlike the status hooks, these don't discard stdout — Claude reads their JSON result as the nudge / completion denial.
