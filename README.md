...
| 󰭹 | Review feedback on **your** PR (yellow) — a reviewer requested changes *or* left review comments (pairs with `C r`, which preloads a fix prompt for it). Fires on any `COMMENTED` / `CHANGES_REQUESTED` review, not just a formal "request changes": GitHub's review *decision* stays `REVIEW_REQUIRED` when someone only comments, so the glyph reads the review states directly. Suppressed once the PR is approved — `C r` still offers the feedback there, deliberately: a glyph sits on the row every frame and would read as "act on this" for a PR that is ready to merge, while `C r` is a key you pressed. |
...
**PR labels.** When the matched PR carries GitHub labels, the meta line carries a tag segment right after the author (before the branch) — a tag glyph (Octicon) followed by the comma-joined label names (e.g. `bug, enhancement`). Like the rest of the meta line it stays muted: label colors are per-repo and don't route through the deck's semantic palette, so only the names render, not GitHub's per-label color. The same tag chip trails each PR in the [`r` review picker](#key-bindings) (capped so a heavily labeled PR can't crowd out the title), and the labels are listed in the merge-confirmation modal (`C m`). Labels are read from the same `gh pr list` / `gh pr view` calls that drive the row glyphs — no extra request.

The status is fetched with a single `gh pr list --state open` call per distinct repo that has at least one non-default workspace, when the deck opens and **on the row refresh thereafter** — so a PR going red, picking up an approval, or being merged from somewhere else reaches its badge on its own. The row poll runs every 5 seconds and the per-repo throttle below holds a repo for a minute, so the effective cadence is a minute; the throttle is what sets it, not merely a bound on it. Only open PRs are listed — the deck only ever displays bulk-list PRs that are open, and listing every recently-closed PR forced GitHub to compute the expensive per-PR CI rollup for ~100 PRs that nothing rendered. Terminal (merged / closed) status for a workspace's PR is filled in the cheap way: a per-PR lookup of the workspace's pinned PR number, plus a write-through right after you merge from the deck. The repos are fetched **concurrently** (bounded so we stay clear of GitHub's rate limits), and within each repo the PR list, the merge-queue lookup, and the per-PR top-ups all run in parallel. The fetch is throttled so the same repo is never re-queried within a minute. The minute starts when the fetch is **asked for**, not when it comes back: a repo whose fetch lands nothing — a network that was down, a rate limit — has to wait its turn like any other, or one repo that never answers turns the poll into a fetch every five seconds for every repo in the batch. The throttle is bypassed for actions that materially change the PR↔workspace mapping: linking a bookmark to an existing workspace, creating a new workspace from a bookmark, and opening a PR review — those refresh the affected repo immediately.

**A project that isn't on GitHub is skipped**, before a single `gh` call. The job reads each repo's `origin` remote — the same lookup `gh` itself does to work out which repository it is talking about, done locally and once — and a repo hosted on GitLab, Bitbucket, or nowhere at all is recorded as having no PRs and reported in the `J` overlay as `<repo> — skipped: gitlab.com`. Such rows simply carry no PR glyphs. Without the check every call against that repo failed identically, once a minute, forever.
...
Detection works by enumerating listening sockets owned by descendants of the workspace's sessions — tmux panes under `awp deck`, the session processes themselves when the deck hosts its own panes (no log scraping, no per-framework config), then picking the numerically lowest port in the range **1024–9999** — typically the HTTP server, since dev-tool sidecars like Vite's HMR socket sit on random high ports. The 1024–9999 cap also keeps ephemeral-range listeners (Claude Code's IPC socket, MCP servers, language servers) from being mistaken for dev URLs. The URL is always `http://localhost:<port>` regardless of whether the server binds to `127.0.0.1` or `0.0.0.0`; the bind address controls who can *reach* the server, not what URL works locally. The line disappears within ~2 seconds of the server stopping.
...
`n` opens one text box, a few lines tall, with nothing above it but the card's own title and the project it will create in. You describe the work the way you would say it out loud — `fix the sidebar cursor bug`, `look at PR 2320`, `spike a jj-backed undo` — and awp asks an agent to turn that into the four things a workspace needs: a directory-safe **name**, a **label** for the deck row, the **prompt** its agent starts on, and the **project**. It is the same translation the captain does when you ask it to start a workspace from an issue. Then it creates it.
...
**It does not hold the deck.** `ctrl+enter` closes the box and you are back on the row list, free to keep working. The agent call and the workspace create that follows it both run in the background, as one `new workspace` chip in the activity bar; the new row appears the way any other backgrounded create's does. There is a 30-second ceiling on the call. Submitting the same sentence twice creates one workspace, not two.

**When it fails** — offline, no agent, a timeout, an answer that will not parse — nothing is created and nothing opens under your hands: the reason goes on the status line, and the sentence you wrote is held for the next `n`, which opens with it already in the box. Fix the wording, or press `ctrl+f` to fill the fields in yourself. awp will not create a workspace from a guess nobody has seen.

**Specifying the fields exactly.** `ctrl+f` opens the structured form and skips the agent. It works from an empty box and from a half-typed sentence, which is carried across as the prompt. The form is unchanged apart from a new optional **Label** field, and every other door to it (creating from a PR row, the `awp workspace open` CLI flow) still opens it directly.
...
| `awp w attention [--json]` | **What wants your attention, and why** — the same rows in the same order the deck's attention scope shows, because it is the same predicate (`internal/deckdata`) printed rather than a second opinion. Spans every project in the store, so it takes no `--project`. This is the read a script or an agent cannot get from `jj` and `gh`: it knows an agent's reported state, its unread mark and whether its session is live |
| `awp w new [--project <p>] [--prompt <text>] [--label <text>] [--bookmark <b>] <name>` | **Create a workspace and return** — no tmux session, no switch. With `--prompt` its agent starts on that prompt (detached), otherwise nothing runs in it until you open its pane. `--label` is what the deck shows for it. This is `open` minus the attach, for callers with no client to switch: see `openRequest.PaneHosted` and #219 |
| `awp w send [--project <p>] <name> <text...>` | **Say something to a workspace's agent** from outside the deck — the same delivery `A` uses, through the same one decision about which substrate holds the agent (`agentPromptSender`). The agent has to be running; this will not start one, because starting one is how a workspace ends up with two |
| `awp w repair [--project <p>] [--dry-run] <name>` | **Tell the agent what's wrong with its PR** — conflicts, failing CI, an out-of-date base, review feedback. The same prompt `C r` builds, so whose PR it is still decides whether the agent is asked to fix and push or to investigate and report (#171). Reads the PR status the deck caches and says so if it is cold rather than guessing |
| `awp ship [--dry-run]` | **This change is done — do what this repo does with done changes.** What that means is the repo's [`ship`](#ship) setting, so the agent that finished the work doesn't have to know which convention the repository follows. `"ship": "main"` rebases the change onto the trunk bookmark, moves the bookmark onto it, and moves the default workspace on — all local, nothing pushed. `"ship": "pull_request"` is named and not built yet, and says so rather than falling back to the other one; unset refuses, because a repo that hasn't stated its convention shouldn't have one guessed. Takes no workspace argument and no `--project`: unlike `repair`, which you do *to* another workspace, ship is what a workspace does to its own change, and a flag naming a different one would only be a way to ship someone else's work by accident. The [`dev_loop`](#dev_loop) gates are re-read as part of shipping rather than taken on trust (the window between the last green check and the ship is exactly where the failure lives), and under the main style a red gate is a stop. A rebase that conflicts leaves trunk where it was and **sends the workspace's agent a prompt to resolve the conflicts** — the same turn-into-repair `awp w repair` makes for a PR |
| `awp w label [--project <p>] <name> [text]` | **What the deck shows for a workspace**, instead of its name. No text clears it. Presentation only — the directory, the session and the bookmark keep the name, and a test enumerates the files allowed to read the field at all. Distinct from `rename`, which moves a directory; renaming keeps the label. **Agents are told about this command** in the preamble awp appends to their system prompt (see [`dev_loop`](#dev_loop)) and run it as `awp w label "$AWP_WORKSPACE" '…'` from inside the workspace, so a row titles itself once the work is clear |
...
| `awp review reply` | Reply on an existing finding's thread: `awp review reply --to <comment-id> [--type comment\|suggestion\|question\|praise] [--proposal] --body <text>`. The reply threads under its parent and flips that parent back to needing your attention, so an exchange stays one item rather than becoming two. The comment id is included in the prompt the agent receives. Names the review it wrote to, and takes `--workspace <name>`, the same way `add` does.<br><br>**`--proposal` marks the reply as a change the agent intends to make, and stops it there.** By default the prompt an agent gets with a finding tells it to **make the change and then reply saying what it did** — a review comment is an instruction, and gating every one of them on a keypress is the copying-between-panes cost this flow exists to remove. `--proposal` is still there for an agent that wants a yes before acting, and `AWP_REVIEW_APPROVAL=1` requires it of every remark (see *Environment* below). Under that gate the prompt tells the agent to reply before changing anything and then stop — and approval used to have no home: the agent's answer was prose in a chat log, saying yes meant leaving the diff to find the agent's tmux window and type it, and nothing recorded that it had happened. A proposal is that exchange written down. The offer is a reply, the approval is a fact in the store, and `A` in the diff view is how you give it. The gate is about *changing code*, not about replying: an agent answering a question, or explaining why the code is the way it is, replies normally and carries on. Only "here is what I would do" needs a yes. A proposal says so on the way out — `proposed to <id> (<id>) in <review> — awaiting approval` — because the two replies mean different things to the caller that just ran the command. |
...
**`--no-attach` is the form a script or an agent uses.** The flow itself was always
non-interactive — fetch the PR, prepare a workspace at its head ref, write the brief,
start a reviewer — but two things around it were not, and both fail by *hanging*
rather than erroring, which reads as work in progress rather than as a mistake. A bare
`awp review` opens a picker over `gh pr list`, which for a caller with nobody at its
stdin is a terminal UI waiting on a keypress that will never come; and the flow ends
by switching your tmux client into the review session, which is why it otherwise
insists on running inside tmux. `--no-attach` takes the half before that: prepare it,
start the reviewer, return. Passing either `--no-attach` or `--project` therefore
*requires* a PR number — the picker is refused outright rather than opened for someone
...
What this repo does with a finished change — the style [`awp ship`](#cli-reference-highlights) follows. One verb rather than two exists because the agent that finished the work should not have to know which convention the repository follows: it knows the change is done, and the repo says what done means there. Without that, an agent in a repo it hasn't seen picks between entirely different sequences of steps and picks wrong.

| Value | What shipping does |
|---|---|
| `"main"` | Rebase the change onto the trunk bookmark (jj's `trunk()`, so a repo that integrates on something not called `main` isn't assumed to), move that bookmark onto the change, and `jj new` the **default workspace** onto the new trunk so the next piece of work doesn't start from yesterday's. **Local only** — nothing is pushed, so a wrong ship costs one `jj bookmark set` rather than a force-push. |
| `"pull_request"` | Push the bookmark and open or update the PR. **Not implemented yet**: a repo configured for it is told so rather than quietly getting the other style. |
| unset | `awp ship` refuses and names the two values. There is no default on purpose — the styles differ in what they make visible to other people, and guessing for a repo that never stated its convention is the failure the verb exists to prevent. |

The change shipped is the workspace's working copy, or the commit below it when the working copy is an empty undescribed commit (the ordinary state of a workspace whose change has been described and left alone). An empty revision, a missing description, or one still marked `wip:` is a stop.

**Gate policy belongs to the style.** Ship re-reads the [`dev_loop`](#dev_loop) gates itself rather than trusting a prior run — the state between the last check and the ship call is exactly where the failure lives — through the same predicate the unit-completion hook uses (`redRequiredGates`), so ship and `awp internal gate check` cannot answer "are the checks green" differently. What happens on red is then the style's own answer, not the caller's: the main style **stops**, because trunk with failing tests on it is everyone's problem, while the pull-request style will **report loudly and allow**, because pushing a branch with a red check so a person can look at it is a real workflow.

**The captain does not run this.** It has no workspace, so it has nothing to ship; its route to a shipped change is `awp workspace send <ws> "ship it"`, which leaves the outward action attributable to the agent that did the work and keeps the captain's refusal list a fixed set of effects rather than one that depends on how a repo is configured.
...
Unset = an inferred default (Go: `gofmt` / `go vet` / `golangci-lint` / `go test` / `go build`, then a `commit` marker). Run `awp watch --suggest` for a ready-to-paste prompt that has an agent inspect the repo and write this block; `awp watch --preamble` prints what an agent started here is actually told — the whole system-prompt append, loop section included, generated from the same config so the observed loop and the instructed loop can't drift.

**Auto-injection.** When the agent is Claude, new coding workspaces launch it with awp's **agent preamble** appended to its **system prompt**: awp writes the text to `~/.awp/agent-preamble/<repo>.md` and launches `claude --append-system-prompt-file <that path>`.

The preamble is two sections that arrive together and are configured apart. The **workspace section** applies everywhere, because it is about awp's own model rather than about how a repo likes to work: it tells the agent that it is in a workspace, that the workspace's row on the deck has a title, and that the title is the agent's to set with `awp w label "$AWP_WORKSPACE" '…'` as the work becomes clear. A workspace's *name* is a slug chosen before the work started — it is the directory, the session and usually the bookmark at once — and the agent is the one that finds out what the work turned out to be. The command is spelled out, environment variable and all: an agent told it may retitle something and left to find the verb invents a plausible flag, runs it, and reports awp as broken.

The **loop section** is added when `dev_loop` is configured (has phases/gates), and carries the generated loop instruction, so every new agent starts already following the loop `awp watch` observes — no manual paste. A repo without a `dev_loop` gets the first section alone; it used to get no preamble at all, which meant its agents were never told the one thing every awp agent can do. It's the system prompt (not a one-shot prompt prefix) so it persists across the session and applies even when the workspace is opened without an initial prompt. Note the instruction is **invisible inside Claude Code** — the system prompt is shown in neither the chat nor the transcript JSONL (that's the point: it keeps the prompt clean). A **reviewer gets the workspace section and not the loop one** — its row has a title like any other, and a review workspace called `pr-2320-jordan-survey-s-a5f9` says nothing a person wants to read, but a reviewer told to work in units, run gates and commit starts doing the author's job on someone else's PR. The two flavors are separate files (`<repo>.md` and `<repo>.review.md`) rather than one rewritten per launch: Claude reads the path at startup rather than when awp hands it over, so a coding agent starting a moment later would replace the text under a reviewer and deliver the loop instruction as a race. Non-Claude agents fall back to no injection either way (`--append-system-prompt-file` is Claude-specific).
...
1. When awp creates or summons a tmux session, it sets `AWP_WORKSPACE`, `AWP_REPO`, and `AWP_REPO_ROOT` on the session env. A hook resolves that triple **three ways, in order**: the process environment, then the tmux session environment (for an agent started before the session env was injected), then — if neither answered — **the working directory**, matched against the workspace paths already in `~/.awp/workspace-state.json`. The third way exists because the first two are injections, and an injection can go missing with no signal at all: a zmx session's environment is fixed when the session is created and attaching re-injects nothing, so a session made by an older awp would quietly stop reporting status and recording gates for as long as it lived. An agent in a workspace pane is *running in* that workspace's directory, so cwd answers for a stale session, a session started outside awp, and whatever the pane environment does next. It is consulted last because it is the least specific — a shell that happens to be `cd`'d into a workspace is in that directory without being that workspace's agent.
...
window) and `C D` (PR description) reached code that opens with *"no tmux
...
them: `c` reviews the change in the deck, `C d` reads the PR description in the
deck.

| Key | Pane | Behind it |
|---|---|---|
| `enter` | agent | **zmx session.** Survives closing the pane, and awp. |
| `a` | captain | **zmx session.** Survives closing the pane, and awp. |
...
**A workspace's name and its label are two things.** `Entry.Name` is identity: the
directory on disk, the zmx session name, and usually the bookmark. That means whoever
creates a workspace has to pick one slug that satisfies a filesystem and a human at
once, and those constraints disagree — `fix-badge-refresh-2` is a fine directory and a
poor sentence. A person splits the difference by habit; an agent has no habit and
...
It is **presentation only**, and that rule needs a guard rather than a comment,
because it is the kind that holds right up until someone needs a name, finds one to
hand, and uses it: the code compiles, the tests pass, and a workspace whose label
differs from its name addresses the wrong thing.
`internal/workspace/display_name_test.go` therefore walks the tree and fails on any
file outside a named allow-list that so much as mentions the field — the same shape
`internal/github/dir_test.go` uses for the same class of problem. Adding a file to that
list is a decision with a reason written beside it: does this code *render* the label,
or *use* it?

Labelling is also deliberately not renaming. A rename moves a directory and a session
and should stay hard; a label is free and reversible, and renaming a labelled
workspace keeps its label, because a rename changes what the work is called rather
than what it is.

**The captain is the one pane with no workspace behind it.** Every other row in
that table is a program run *in* a workspace's working copy, told which workspace it
belongs to through `AWP_WORKSPACE`. The captain has no working copy and no entry in
...
confusion the captain exists not to have. And no `AWP_WORKSPACE`, because every awp
hook opens by asking for it: with `captain` in there, the captain's agent would
report status and record gates against a workspace nothing has an entry for, and the
...
is about reading someone else's change. The captain's is a third, and what it says
is the set of things the captain is and is not allowed to do.
...
**The menu needs nothing from the terminal.** `ctrl+b` is `0x02` on a plain
terminal and under the Kitty keyboard protocol alike.
...
`ctrl+b` is the tmux prefix and readline's backward-char, so **a pane's program
stops receiving it**. That is the deliberate cost: awp reserves exactly two keys,
and a reserved key is only reserved if something gives it up. `ctrl+\` was free
because it is SIGQUIT and nothing interactive binds it; there is no second key that
is both free and comfortable, so the menu takes one that is comfortable. (`ctrl+space`
was tried in between and is worse than either: macOS binds it system-wide to
switching input sources, so it never reaches the terminal at all.)
...
be done about it from this side. A held `ctrl+b` is harmless on every terminal,
...
on someone else's PR. No `pr description` tmux window is opened either — `C d`
...
One behaviour switch, off unless set:

| Variable | Effect |
|----------|--------|
| `AWP_REVIEW_APPROVAL=1` | Require an agent to get your **yes** before it changes code in response to a review remark. Off by default: the prompt tells the agent to make the change and reply saying what it did, since a comment is an instruction and gating every one of them costs a keypress per remark — which is the cost the send-to-agent flow exists to remove. Set it and the prompt reverts to "reply before changing anything, add `--proposal`, then stop", answered with `A` in the diff view. Read per prompt, not at startup, so flipping it does not need the deck restarted; an off-ish value (`0`, `false`, `no`, `off`) is the default and anything else is on. The machinery is unaffected either way — an agent may always send `--proposal` when it wants a yes, and `A` always approves one. |
