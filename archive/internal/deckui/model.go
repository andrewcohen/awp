...
func (m Model) Update(msg tea.Msg) (model tea.Model, cmd tea.Cmd) {
	...
	case tea.KeyPressMsg:
		...
		case key.Matches(msg, km.LastSession):
			...
			// other tmux client to switch to, and the panes you were in are
			// something only the deck knows about. Everything below is the tmux
			// deck, where L is `tmux switch-client -l` — the same alternation, one
			// substrate over.
			if cmd, handled := (&m).alternatePane(); handled {
				return m, cmd
			}
			if m.handler == nil {
				return m, nil
...
func (m Model) trigger(a Action, arg string) (tea.Model, tea.Cmd) {
	...
	if item.Virtual {
		...
		//     PR branch, so you land in a normal working workspace rather
		//     than a review workspace primed for reviewing someone else.
...
func (m Model) View() tea.View {
	...
	if _, diff := m.active.(*diffModal); diff {
		// The viewer's own wheel (see ui.Model.WheelAt). Same trade as the split's
		// divider below: it costs the terminal's drag-to-select while the diff is up,
		// and the deck is asking on its own behalf rather than a program's.
