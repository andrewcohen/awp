...
func TestRunOpenParsesBookmarkWithoutName(t *testing.T) {
	svc := &fakeService{}
	app := NewApp(svc, &bytes.Buffer{})
	if err := app.Run([]string{"w", "open", "-b", "coworker/no-default-standard-delivery-preference"}); err != nil {
		t.Fatalf("Run returned error: %v", err)
	}
	if svc.openName != "" || svc.openBookmark != "coworker/no-default-standard-delivery-preference" {
