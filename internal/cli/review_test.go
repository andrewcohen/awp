...
func TestRewriteFetchURL(t *testing.T) {
	const fallback = "https://github.com/CoWorker/awp.git"
	cases := []struct {
		name   string
		origin string
		want   string
		...
		// username. Must preserve ssh:// + user so SSH-key auth works.
		{
			...
			want:   "ssh://git@github.com/CoWorker/awp.git",
		},
		{
		...
		},
		{
			...
			want:   "https://github.com/CoWorker/awp.git",
		},
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			got := rewriteFetchURL(c.origin, "CoWorker", "awp", fallback)
			if got != c.want {
				t.Errorf("rewriteFetchURL(%q, CoWorker, awp) = %q, want %q", c.origin, got, c.want)
			}
		})
	}
}
