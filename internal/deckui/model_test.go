...
// A repair-originated prompt (`p r`) submits through the same generic
// ActionSendPrompt path as the blank `A` dialog. It once branched to a
// ReviewReloader that reloaded a stale external review window before sending;
// that tool is retired, so there is one submit path to test.
func TestRepairPromptSubmitSendsPrompt(t *testing.T) {
	var gotAction Action
	item := Item{ProjectName: "growth", WorkspaceName: "review-430"}
	model := New([]Item{item}, func(req ActionRequest) error {
		gotAction = req.Action
		return nil
	})
	var initCmd tea.Cmd
	model.promptForm, initCmd = newPromptForm(item, "please re-review")
	_ = initCmd
